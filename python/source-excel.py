    ...
    try:
        for line in source:
            if is_link : 
               ...
               msg = cgi.escape(line[0:pos_end]).decode('gbk', errors='ignore').encode('utf-8')
               target.write(msg+')')
               target.write(u'  \n   \n')   
               is_link = False
            if len(line)>690 and line[63:67] == 'xl24':
                ...
                if date == thedate and msgtype == unicode('照片壁纸','utf8'):
                    ...
                    target.write('!['+ datetime + '](http://static.cocolian.cn/img/'+ date.replace('-', '')+'_'+ time.replace(':','') + '.png' + ') \n   \n')
...
## 寻找最近日期的xls文件
    ...
    thedate = datetime.datetime(2017, 12, 17)

    while thedate < datetime.datetime(2017, 12, 31) : 
...
# gen_files(u'/表格格式/支付产品开发交流群.xls', 'devarch')
