...
target.write(u'wechatno,姓名, 频率, 最后发言时间 \n')              
freq ={}
last = {}
names = {}
try:
	for line in source:
		if len(line)>690 and line[63:67] == 'xl24':
			...
			if names.has_key(wechat_no) :
				if name in names[wechat_no] :
					name = name 
				else: 
					names[wechat_no] = names[wechat_no] + " | " + name
			else :
...
for wechat_no in freq:
	target.write('\"' + wechat_no + "\"\t\""+ names[wechat_no] + "\"\t" + str(freq[wechat_no]) +"\t"+ last[wechat_no]+'\n')
